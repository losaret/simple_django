...
class TestCategoryView:
    def test_post_success(self, client, test_user):
        ...
        assert Category.objects.filter(
            user=test_user, name="Новая тестовая категория"
        ).exists()
    ...
    def test_post_duplicate_ignored(self, client, test_user, test_category):
        ...
        count_before = Category.objects.filter(user=test_user).count()
        response = client.post(url, data=data)
        # Считаем количество после
        count_after = Category.objects.filter(user=test_user).count()
    ...
    def test_post_update(self, client, test_user, test_category):
        ...
        assert response.status_code == 302
        assert Category.objects.filter(
            user=test_user, name="Новое тестовое имя"
        ).exists()
...
class TestProductCardView:
    def test_post_success(self, client, test_user, test_category):
        ...
        uploaded_file = SimpleUploadedFile(
            "test.gif", gif_data, content_type="image/gif"
        )
        data = {
        ...
        response = client.post(url, data=data)
        assert response.status_code == 302
        assert ProductCard.objects.filter(
            user=test_user, comment="Новая тестовая карточка"
        ).exists()

    def test_post_delete(self, client, test_user, test_category, test_product_card):
        """Проверка: успешное удаление карточки."""
        assert ProductCard.objects.count() == 1
        client.force_login(test_user)
